...
def _BLASTsearch_(transcripts, outF, blastdb, coverage, cpu):
    annotated = {}
    nonannotated = {}
    if "," in blastdb: #if it has more than one blastDB
        ...
        for n in range(0, len(dblist)):
            ...
            os.system("blastx -qcov_hsp_perc "+coverage+" -num_threads "+cpu+" -evalue 1e-3 -max_target_seqs 5 -db "+dblist[n]+" -query "+temp+" -out "+OUT+" -outfmt \'6 qseqid qlen sseqid slen pident length mismatch qstart qend sstart send evalue bitscore stitle\'")
        ...
        OUT = outF+"blast.out"
        os.system("blastx -qcov_hsp_perc "+coverage+" -num_threads "+cpu+" -evalue 1e-3 -max_target_seqs 5 -db "+blastdb+" -query "+transcripts+" -out "+OUT+" -outfmt \'6 qseqid qlen sseqid slen pident length mismatch qstart qend sstart send evalue bitscore stitle\'")
...
#>>>>Perform annotation
def _Annotation_(transcripts, outF, blastdb, coverage, busco, pfam, cpu):
    annotated, nonannotated = _BLASTsearch_(transcripts, outF, blastdb, coverage, cpu)
    if busco != None:
    ...
    for k in nonannotated.keys():
        if k in busco_annotated.keys() and k in pfam_annotated.keys():
            SEQ = nonannotated[k]
            seq = "\n".join([SEQ[n:n+100] for n in range(0, len(SEQ), 100)])
            OUT.write(">"+k+" "+busco_annotated[k]+" "+" ".join(pfam_annotated[k])+"\n"+seq+"\n")
        ...
        if k not in busco_annotated.keys() and k in pfam_annotated.keys():
            SEQ = nonannotated[k]
            seq = "\n".join([SEQ[n:n+100] for n in range(0, len(SEQ), 100)])
            OUT.write(">"+k+" "+" ".join(pfam_annotated[k])+"\n"+seq+"\n")
        else:
            SEQ = nonannotated[k]
            seq = "\n".join([SEQ[n:n+100] for n in range(0, len(SEQ), 100)])
            OUT.write(">"+k+"\n"+seq+"\n")
...
def __main__():
    ...
    parser.add_option("-d", "--db", dest="blastdb", help="Mandatory - path to blast database generated with protein sequences, /path/to/proteinDB; the user can set more than one proteinDB (i.e., from one to \"n\" databases) separeted by a comma (e.g., path/to/proteinDB1,path/to/proteinDB2, ... ,path/to/proteinDBn)", metavar="path", default=None)
    parser.add_option("-f", "--coveragefilter", dest="coverage", help="Optional - threshold value used as the minimum coverage in the BLAST search [default=90]", metavar="int", default="90")
    ...
    \'-f\', set the filter threshold considered in the BLAST search [default=90].
